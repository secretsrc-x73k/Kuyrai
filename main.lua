...
if not ModernV2 then
    ...
    local loaderOk, loaderResult = pcall(function()
        local source = game:HttpGet("https://raw.githubusercontent.com/secretsrc-x73k/Kuyrai/refs/heads/main/gui/main.lua")
    ...
    if loaderOk then
        ...
        local fallbackOk, fallbackResult = pcall(function()
            local source = game:HttpGet("https://raw.githubusercontent.com/secretsrc-x73k/Kuyrai/refs/heads/main/gui/main2.lua")
            local fn, compileErr = loadstring(source)
            if not fn then error(compileErr) end
            return fn()
        end)
        if fallbackOk then
            ...
            warn("[REAPER] Failed to load ModernV2 from all sources:", fallbackResult)
        end
    end
end
...
function setupSpearAimbotBtn()
    ...
    print("[REAPER] Spear Aimbot Button created successfully")
end
